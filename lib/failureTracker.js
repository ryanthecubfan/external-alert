...
exports.addFailure = function(workflowConfig, details) {
    ...
    if(arr.length >= workflowConfig.threshold) {
        ...
        log.write(component, arr.length + " consecutive failures in workflow '" + workflowConfig.workflowName + "'.  Sending email to '" + workflowConfig.mailRecipients + "'", "CRITICAL");
        mailer.sendMail(subject, buildBody(failures[workflowConfig.workflowName]), workflowConfig.mailRecipients);
...
exports.clearFailures = function(workflowConfig) {
    log.write(component, "Clearing failures for workflow '" + workflowConfig.workflowName + "'")
    failures[workflowConfig.workflowName] = null;
}
