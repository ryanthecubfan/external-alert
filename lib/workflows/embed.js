...
var url = process.env.PREVIEW_URL || "http://embed.gettyimages.com/preview/1765189";
var timer = process.env.PREVIEW_TIMER || 60000;
    ...
    return { 
        action: function() {
            ...
            HttpLoader.execute(url)
                ...
                .then(HttpLoader.execute)
                .then(function(data) {
                    console.log("Successfully created embed via '" + url + "', viewed the iframe, and loaded the image.")
                })
                .catch(function(message) {
                    console.log("****************Embed Workflow Error****************");
                    console.log(message);
                    AlertManager.sendMail("Embed Preview Failure", "Embed Preview failed: \r\n\r\nReason:\r\n\r\n" + message, mailRecipients);
                });
        }, 
        interval: timer
    };
